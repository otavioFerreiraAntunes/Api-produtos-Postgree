...
// e formatar dados para exibição (Moeda, Categoria).
// ═════════════════════════════════════════════
...
/**
 * Formata valor para moeda brasileira: 1500.5 → "R$ 1.500,50"
 */
 ...
 * IMPORTANTE: quando um nome contém apóstrofo (ex: "Coxinha d'água"),
...
async function carregarProdutos() {
    ...
    try {
        // Envia GET para /produtos — sem body, sem headers especiais
...
async function deletarProduto(id) {
    console.log("ID que o Front está enviando:", id)
    if (!confirm('Tem certeza que deseja deletar este produto?')) return;

    try {
        // Tente remover a primeira barra ou garantir o caminho absoluto
        const resposta = await fetch(`produtos/${id}`, {
            method: 'DELETE'
        });
 ...
 * 1. Formata os dados para exibição (Preço, Estoque)
 * 2. Cria uma linha <tr> com botões de Editar e Deletar
 * 3. Insere tudo no div#productsList do HTML
 */
function exibirTabela(produtos) {
  ...
  `;
    ...
    produtos.forEach(produto => {
        ...
        // que quebrariam o onclick="editarProduto('Coxinha d\'água', ...)"
        html += `
      <tr>
        <td>#${produto.id}</td>
        <td>${produto.nome}</td>
...
// Contém duas funções:
//   buscarProdutos()  → faz o fetch para as rotas de busca
//                       do back-end (/produtos/nome/:nome
//                       ou /produtos/:id) e exibe os resultados
//   filtrarProdutos() → lê os campos do formulário de busca
...
async function buscarProdutos(tipo, valor) {
    ...
    try {
        ...
        if (tipo === 'nome') {
            ...
            // Ex: "Café Moído" → "Caf%C3%A9%20Mo%C3%ADdo"
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('productForm').addEventListener('submit', async function (e) {
        ...
        const dados = {
            ...
            // parseFloat converte a string do input para número decimal (preço)
            preco: parseFloat(document.getElementById('preco').value),
            // parseInt converte a string do input para número inteiro (estoque)
